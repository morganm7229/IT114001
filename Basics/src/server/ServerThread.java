...
public class ServerThread extends Thread {
	...
	public void addMuted(String toAdd) {
		boolean alreadyInMuted = false;
		for (int i = 0; i < muted.size(); i++) {
			if (muted.get(i).equals(toAdd)) {
				alreadyInMuted = true;
			}
		}
		if (alreadyInMuted == false) {
			muted.add(toAdd);
		}
	}

	public void removeMuted(String toRemove) {
		for (int i = 0; i < muted.size(); i++) {
			if (muted.get(i).equals(toRemove)) {
				muted.remove(i);
			}
		}
	}
	
	public boolean checkMuted(String toCheck) {
		boolean isMuted = false;
		for (int i = 0; i < muted.size(); i++) {
			if (muted.get(i).equals(toCheck)) {
				isMuted = true;
			}
		}
		return isMuted;
